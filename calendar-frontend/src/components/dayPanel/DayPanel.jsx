...
export default function DayPanel({ selectedDay, onClose, weatherDay, suggestion, refetch, dayEvents, selectedDayHolidays, isTransitioning }) {
    ...
    return (
        <div className="w-96 shrink-0 bg-white rounded-2xl shadow-sm p-4 flex flex-col gap-3 overflow-hidden"
            ...
            {view === 'form' ? (
                ...
                <div className="flex-1 min-h-0 overflow-hidden relative animate-fade-in"
                    key={selectedDay?.toISOString()}>
                    {/* Day view - slides up when eventsOpen */}
                    <div className="absolute inset-0 flex flex-col transition-transform duration-300"
                        style={{ transform: eventsOpen ? 'translateY(-100%)' : 'translateY(0)' }}>
